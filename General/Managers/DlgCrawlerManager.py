...
from content_fetchers import BROWSER_HEADERS, BROWSER_USER_AGENT, fetch_with_requests
from DatabaseOperation.SQLAlchemy.DatabaseModels import (
    AuditAction,
    FetchResult,
    LspMaster,
    dlg_raw_from_dict,
...
class DlgCrawlerManager:
    ...
    def __init__(self) -> None:
        ...
        self.config_manager = DlgCrawlerConfigManager()
        self.raw_manager = DlgRawManager()
    ...
    def scrape_one(self, source: LspMaster) -> Tuple[str, Optional[str], Optional[str], List[Dict[str, Any]]]:
        ...
        rules = load_rules(config.rules_json) if config.rules_json else {}
        ...
        fetch = self._execute_fetch(source, config, pre_click_js)

        if config.parse_hint == "ocr_simple" and not looks_like_pdf(fetch):
    ...
    def _parse_rows(
        ...
        if parse_hint == "plain_text":
            return extract_dlg_from_plain_text(fetch.body, lsp_name=source.lsp_name, scrape_ts=scrape_ts, rules_json=config.rules_json)
    ...
    # ------------------------------------------------------------------
    def _fetch_with_playwright(self, url: str, timeout_ms: int = 60_000, pre_click_js: str | None = None) -> FetchResult:
