...
class DisclosureUrlFinderService:
    """Service for finding DLG disclosure URLs from homepage URLs."""
    ...
    DLG_CONTENT_RE = re.compile(
        ...
        r"guidelines?\s+on\s+default\s+loss\s+guarantee|"
        r"disclosure\s+under.*?default\s+loss\s+guarantee|"
    ...
    DLG_CONTENT_LOOSE_RE = re.compile(
        ...
        r"guideline.*?default.*?loss|disclosure.*?default.*?loss|"
    ...
    def find_dlg_disclosure_url(
        ...
        """
        homepage_url = self._normalize_home(homepage_url)
        origin = self._origin(homepage_url)
        ...
        for path in self.COMMON_DISCLOSURE_PATHS:
            cand = urljoin(origin + "/", path.lstrip("/"))
            if self._probe_url_exists(session, cand):
                # If it exists AND looks relevant, check content before returning
                if self._score_candidate(cand) >= 10 and self._check_dlg_content(session, cand):
        ...
        def consider(url: str, anchor_text: str = ""):
            ...
            if not url or self._is_asset(url):
                return
            sc = self._score_candidate(url, anchor_text)
            if sc > best_score:
                best_score = sc
                best_url = url
        ...
        while q and pages < max_pages:
            ...
            if not self._same_site(origin, page_url) or self._is_asset(page_url):
                continue

            html, final_url, status = self._fetch_html(session, page_url, timeout=timeout)
            pages += 1
            crawled_pages += 1
            if delay_s:
                time.sleep(delay_s)
            ...
            if not html:
                ...
                if page_url == homepage_url and PLAYWRIGHT_AVAILABLE:
                    try:
                        html = self._fetch_with_playwright(page_url)
                        if html:
                            soup = BeautifulSoup(html, "html.parser")
                        else:
                            continue
                    except Exception as e:
                        self.logger.debug(f"{self._get_user_info()} Failed to fetch homepage with Playwright: {str(e)[:100]}")
                        continue
                else:
                    continue
            else:
            ...
            if page_url == homepage_url and len(all_links) == 0 and PLAYWRIGHT_AVAILABLE:
                try:
                    ...
                    self.logger.debug(f"{self._get_user_info()} Failed to fetch JS-rendered homepage with Playwright: {str(e)[:100]}")
            ...
            for a in soup.find_all("a", href=True):
                ...
                if self.DISCLOSURE_KEYWORDS_RE.search(href) or self.DISCLOSURE_KEYWORDS_RE.search(
                        txt) or self.STRONG_URL_HINTS_RE.search(abs_u):
                    consider(abs_u, txt)
                    total_links_found += 1
            ...
            if best_url and best_score >= 14:
                ...
                if self._check_dlg_content(session, best_url, use_loose=use_loose):
            ...
            for a in soup.find_all("a", href=True):
                ...
                if self._is_bad_href(href):
                    continue
                abs_u = urljoin(final_url, href)
                if self._is_asset(abs_u) or not self._same_site(origin, abs_u):
                    continue
        ...
        if best_url and best_score > 0:
            ...
            if self._check_dlg_content(session_final, best_url, use_loose=use_loose):
        ...
        pdf_url, pdf_reason = self._search_pdfs(session_fallback, homepage_url, origin)
        if pdf_url:
        ...
        sitemap_url, sitemap_reason = self._search_sitemap(session_fallback, origin)
        if sitemap_url:
        ...
        special_url, special_reason = self._search_special_sections(session_fallback, homepage_url, origin)
        if special_url:
        ...
        if total_links_found > 0:
            ...
            if best_url and best_score > 0:
                details.append(
                    f"best candidate score: {best_score:.1f} at {best_url[:60]}... (failed DLG content validation)")
            if high_score_candidates:
                candidate_list = "; ".join(
                    [f"{url[:40]}... (score: {sc:.1f})" for url, sc, txt in high_score_candidates[:3]])
                details.append(f"top candidates: {candidate_list}")
        else:
            details.append(
                "no disclosure-related links found (site may use JavaScript navigation or non-standard structure)")
    ...
    def _same_site(self, origin: str, candidate: str) -> bool:
        o = urlparse(origin)
        c = urlparse(candidate)
        return (not c.netloc) or (c.netloc.lower() == o.netloc.lower())

    def _is_asset(self, url: str) -> bool:
        return bool(self.ASSET_EXT_RE.search(url))
    ...
    def _fetch_html(self, session: requests.Session, url: str, timeout: int = 20) -> Tuple[Optional[str], str, int]:
        ...
        try:
            ...
            self.logger.debug(f"{self._get_user_info()} Failed to fetch HTML from {url}: {str(e)[:100]}")
            return None, url, 0

    def _probe_url_exists(self, session: requests.Session, url: str, timeout: int = 15) -> bool:
        """Lightweight existence check using GET."""
        try:
            r = session.get(url, timeout=timeout, allow_redirects=True)
            ...
            self.logger.debug(f"{self._get_user_info()} Failed to probe URL {url}: {str(e)[:100]}")
            return False

    def _check_dlg_content(self, session: requests.Session, url: str, timeout: int = 15,
                           use_loose: bool = False) -> bool:
        """
        ...
        """
        regex_pattern = self.DLG_CONTENT_LOOSE_RE if use_loose else self.DLG_CONTENT_RE
        ...
        try:
            ...
            if "application/pdf" in ctype or url.lower().endswith(".pdf"):
                try:
                    ...
                    self.logger.warning(f"{self._get_user_info()} Failed to extract PDF content from {url}: {str(e)[:100]}")
                    return False
            ...
            self.logger.debug(f"{self._get_user_info()} Failed to check DLG content at {url}: {str(e)[:100]}")
            return False

    def _score_candidate(self, url: str, anchor_text: str = "") -> float:
        u = (url or "").lower()
        ...
        if self.STRONG_URL_HINTS_RE.search(u):
            score += 10

        # Keyword in URL and/or text
        if self.DISCLOSURE_KEYWORDS_RE.search(u):
            score += 5
        if self.DISCLOSURE_KEYWORDS_RE.search(t):
            score += 5
        ...
        if any(x in u for x in
               ("/legal", "/policies", "/disclaimer", "/compliance")) and "dlg" not in u and "dlg" not in t:
            score -= 3

        return score

    def _search_pdfs(self, session: requests.Session, homepage_url: str, origin: str) -> Tuple[Optional[str], str]:
        """Strategy: Search specifically for PDF files that might contain DLG disclosures"""
        try:
            html, _, _ = self._fetch_html(session, homepage_url)
            if not html:
                return None, ""
            ...
            for a in soup.find_all("a", href=True):
                ...
                if ".pdf" in href.lower():
                    ...
                    score = self._score_candidate(abs_url, text)
            ...
            if pdf_links:
                ...
                if self._check_dlg_content(session, best_pdf, use_loose=True):
                    return best_pdf, f"Fallback: PDF search (score: {pdf_links[0][1]:.1f})"
        except Exception as e:
            self.logger.error(f"{self._get_user_info()} Error in PDF search fallback for {homepage_url}: {str(e)[:150]}")

        return None, ""

    def _search_sitemap(self, session: requests.Session, origin: str) -> Tuple[Optional[str], str]:
            ...
            for sitemap_url in sitemap_urls:
                try:
                    ...
                    for loc in urls:
                        url = loc.text
                        if url and any(
                                keyword in url.lower() for keyword in ['dlg', 'disclosure', 'compliance', 'rbi']):
                            if self._check_dlg_content(session, url, use_loose=True):
                                return url, "Fallback: Found in sitemap.xml"
                except Exception as e:
                    self.logger.debug(f"{self._get_user_info()} Error parsing sitemap {sitemap_url}: {str(e)[:100]}")
                    continue
        except Exception as e:
            self.logger.error(f"{self._get_user_info()} Error in sitemap search fallback: {str(e)[:150]}")

        return None, ""

    def _search_special_sections(self, session: requests.Session, homepage_url: str, origin: str) -> Tuple[
        Optional[str], str]:
            ...
            try:
                ...
                for a in soup.find_all("a", href=True):
                    ...
                    if any(kw in href.lower() or kw in text.lower() for kw in ['dlg', 'default loss', 'portfolio']):
                        ...
                        if self._check_dlg_content(session, abs_url, use_loose=True):
                            return abs_url, f"Fallback: Found in {path}"
            except Exception as e:
                self.logger.debug(f"{self._get_user_info()} Error searching special section {path}: {str(e)[:100]}")
                continue
    ...
    def _fetch_with_playwright(self, url: str) -> Optional[str]:
        ...
        try:
            ...
            self.logger.warning(f"{self._get_user_info()} Failed to fetch with Playwright from {url}: {str(e)[:150]}")
            return None
